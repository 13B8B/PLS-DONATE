...
local xspin, nx, RainbowTextActivated = 0, 0, false
	
local fonts = {
	"AmaticSC", "Antique", "Arcade", "Arial", "ArialBold", "Bangers", "Bodoni", "Cartoon", "Code", "Creepster", "DenkOne",
	"Fantasy", "Fondamento", "FredokaOne", "Garamond", "Gotham", "GothamBlack", "GothamBold", "GothamMedium", "GrenzeGotisch",
	"Highway", "IndieFlower", "JosefinSans", "Jura", "Kalam", "Legacy", "LuckiestGuy", "Merriweather", "Michroma", "Nunito",
	"Oswald", "PatrickHand", "PermanentMarker", "Roboto", "RobotoCondensed", "RobotoMono", "Sarpanch", "SciFi", "SourceSans",
	"SourceSansBold", "SourceSansItalic", "SourceSansLight", "SourceSansSemibold", "SpecialElite", "TitilliumWeb", "Ubuntu"
}  
...
for i = 0, 255, 4 do
	local r = i > 191 and 255 or i < 64 and 0 or (i - 64) * 4
	local g = i < 64 and 255 or i > 191 and 0 or (191 - i) * 4
	local b = i < 128 and i * 4 or i > 191 and (255 - (i - 192) * 4) or 255

	table.insert(BetterRainbowColorHex, string.format("#%02X%02X%02X", r, g, b))
end
...
if queueonteleport then
  queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/13B8B/PLS-DONATE/main/Loadstring.lua'))()")
end
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/13B8B/PLS-DONATE/main/Interferance"))()
...
getgenv().settings = {} 
if isfile("NexusPlsDonateSettings.txt") then
	local sl, er = pcall(function()
		getgenv().settings = game:GetService('HttpService'):JSONDecode(readfile('NexusPlsDonateSettings.txt'))
	end)
	if er ~= nil then
		task.spawn(function()
...
local sNames = {
	"textUpdateToggle", "textUpdateDelay", "serverHopToggle",
	"serverHopDelay", "hexBox", "goalBox", "webhookToggle",
	"webhookBox", "danceChoice", "thanksMessage", "signToggle",
	"customBoothText", "signUpdateToggle", "signText", "signHexBox",
	"autoThanks", "autoBeg", "begMessage", "begDelay", "fpsLimit",
	"render", "thanksDelay", "vcServer", "donationJump",
	"AlternativeHop", "autoNearReply", "boothPosition",
	"standingPosition", "AnonymousMode", "boothSwitcher",
	"serverHopAfterDonation", "jumpsPerRobux", "staffHopA",
	"spinSet", "boothTop", "spinSpeedMultiplier", "webhookAfterSH",
	"minimumDonated", "webhookType", "OutfitLoader", "fontFace",
	"fontSize", 'noFont', 'taggedBoothHop', 'rainbowText',
	'helloResponce', 'botResponce', "donateResponce",
	"otherResponce", "scamResponce", "pingEveryone", "pingAboveDono"
}
...
local sValues = {
    ...
    false, "✅ 1 ROBUX DONATED = $D JUMP ✅", false, "your text here", "#ffffff",
    true, true, {"1R$ = 1 JUMP", "Jumping for donations!"}, 300, 60, false, 3,
    true, true, false, false, 3, 'Front', false, false, false, 1,
    true, false, false, 1, false, 0, 'New', false, 'SciFi', 5, false, false,
    false, {'Hi', 'Sup', 'Hello'},
...
if #getgenv().settings ~= sNames then
	...
	writefile('NexusPlsDonateSettings.txt', httpservice:JSONEncode(getgenv().settings))
end
...
local RandomName = "NexusPleaseDonateServerHop"
...
local Window = library:AddWindow("Nexus : Pls Donate",
  {
...
local PromtOutfit = otherTab:AddSwitch('Load Promt Outfit', function(bool)
    getgenv().settings.OutfitLoader = bool
    saveSettings()
    task.spawn(function()
        while task.wait() do  
        if getgenv().settings.OutfitLoader then
            local promptWearLastOutfit = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("PromptWearLastOutfit")
            if promptWearLastOutfit then
                promptWearLastOutfit.PromptResult:FireServer(true)
            else
                warn("PromptWearLastOutfit does not exist")
            end
            end
        end
    end)
end)
PromtOutfit:Set(getgenv().settings.OutfitLoader)
...
task.spawn(function()
	...
	Players.LocalPlayer.CharacterRemoving:Connect(function()
		if getgenv().settings.spinSet then serverHop() end
	end)
	for _, v in pairs(Players:GetPlayers()) do
		if v ~= Players.LocalPlayer and v:FindFirstChild('leaderstats') then
			raisedV = math.max(raisedV, v.leaderstats.Raised.Value)
		end
	end
	if raisedV < getgenv().settings.minimumDonated then serverHop() end
end)

task.spawn(function()
	while task.wait(5) do
		saveSettings()
		local rootPart = Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChildOfClass('Humanoid').RootPart
		if rootPart and (rootPart.Position - positionX.Position).Magnitude > 1100 then serverHop() end
	end
end)

if getgenv().settings.webhookAfterSH then webhook('hi hello ' .. Players.LocalPlayer.DisplayName .. ' (' .. Players.LocalPlayer.Name .. ') serverhopped') end
...
local randombotmsgs = { 'no im not a bot', 'why do yall think im a bot :(', 'bro im not a bot', 'bruh shut up im a real human' }
...
msgdone.OnClientEvent:Connect(function(msgdata)
	...
	if not getgenv().settings.autoNearReply or string.find(message, 'donates') or string.find(message, 'spamming') then return end
	local plrChatted = game:GetService('Players')[speaker]
	if plrChatted == Players.LocalPlayer or not plrChatted or not plrChatted.Character or not plrChatted.Character.Humanoid.RootPart then return end
	local root = plrChatted.Character.Humanoid.RootPart
